...
public class SharedNoteDaoImpl implements SharedNoteDao {
    ...
    @Override
    public List<SharedNote> filterSharedNotesWithMeAll(long userid, String title, String initDate, String endDate) throws Exception {
        List<SharedNote> result = new ArrayList<>();
        Connection conn = Database.getConnection();
        PreparedStatement all = conn.prepareStatement("SELECT shared_note, s.user_id, email, username, password, s.note_id, user_iduser, title, body, creationDate, lastModificationDate FROM sharedNote AS s INNER JOIN user ON user.user_id = s.user_id INNER JOIN note ON note.note_id = s.note_id WHERE s.user_id = ? AND title LIKE ? AND creationDate > ? AND lastModificationDate < ?");
        all.setLong(1, userid);
        //2020-11-12 00:00:00
        //2020-11-12 23:59:59
