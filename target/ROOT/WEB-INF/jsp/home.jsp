        ...
        <c:forEach var="note" items="${notes}">
            <c:choose>
                <c:when test="${note.user.iduser == userid}">
                    <c:choose>
                        <c:when test="${typeNote == 'compartit'}">
                            <div class="card" style="width: 18rem; background-color: #53b3cb; color: white">
                                <a href="/detail?id=${note.idnote}">
                                    <div class="card-body">
                                        <h5 class="card-title">${note.title}</h5>
                                        <p class="card-text"
                                           style="display: -webkit-box; -webkit-line-clamp: 11; -webkit-box-orient: vertical;overflow: hidden;">${note.body}</p>
                        ...
                        <c:otherwise>
                            <div class="card" style="width: 18rem; background-color: #f9c22e; color: black">
                                <div class="card-body">
                                    <h5 class="card-title">${note.title}</h5>
                                    <h6 class="card-subtitle mb-2 text-muted">${note.user.username}</h6>
                                    <p class="card-text"
    ...
    <nav id="pagination" aria-label="Page navigation example">
        <ul class="pagination">
            <c:if test="${currentPage != 1}">
                <li class="page-item">
                    <a class="page-link principalButton" href="/home?currentPage=${currentPage-1}&${filterURL}">Previous</a>
                </li>
            </c:if>
